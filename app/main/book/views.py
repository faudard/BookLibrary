...
def edit(book_id):
    ...
    return render_template("book_edit.html", form=form, book=book, title=u"Editor's information ")
...
def add():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(u'books %s added to library!' % new_book.title, 'success')
...
def delete(book_id):
    ...
    db.session.commit()
    flash(u'Successfully deleted the book,the user can no longer view the book', 'info')
...
def put_back(book_id):
    ...
    db.session.add(the_book)
    db.session.commit()
    flash(u'The book was successfully restored, and the user can now view the book', 'info')
    return redirect(request.args.get('next') or url_for('book.detail', book_id=book_id))
